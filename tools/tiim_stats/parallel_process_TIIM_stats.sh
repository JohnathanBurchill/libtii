...
# TIILIB: tools/tiim_stats/parallel_process_TIIM_stats.sh
...
# processes TII NOM and TIC Level 0 binary file image statistics for specified satellite for a range of dates
...
while [ $cpuNum -lt $numCpus ]; do
	cpuNum=$((cpuNum + 1))
	xterm -geometry 80x1 -e "process_TIIM_stats.sh ${satellite} ${dateToProcess} ${stopDate} $numCpus" &
