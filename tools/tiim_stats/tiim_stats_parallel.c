/*
    ...
    TIIM processing tools: tools/tiim_stats/tiim_stats_parallel.c
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            fprintf(stdout, "tiim_stats_parallel version %s.\n", SOFTWARE_VERSION);
	...
	if (argc !=  6)
	{
		printf("usage:\t%s <SatelliteLetter> <exportDirectory> <startyyyymmdd> <endyyyymmdd> <nthreads>\n\t\tparallel processes Swarm TII L0 imagery to generate daily image statistics.\n", argv[0]);
	...
	int minutes = 0;
	int hours = 0;
	mvprintw(TIIM_STATS_LABEL, "TIIM_STATS processor");
...
void *runThread(void *a)
{
	...
	// exec tiim_stat command 
	int status = 0;
	char command[5*FILENAME_MAX+256];
	sprintf(command, "tiim_stats %s%s %s 2>/dev/null", args->satLetter, args->date, args->exportDir);
