...
# TIILIB: tools/lp_sweeps/process_lp_sweeps.sh
...
while [ "$dateToProcess" -le "$stopDate" ]; do
	...
	draw_progress_bar $daysProcessed $daysToProcess "days: LP_SWEEPS ${satellite} $datestring"
 	lp_sweeps ${satellite}${year}${month}${day} ${outDir} ${intervalSeconds}
