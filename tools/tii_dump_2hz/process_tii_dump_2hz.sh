...
# TIILIB: tools/tii_dump_2hz/process_tii_dump_2hz.sh
...
while [ "$dateToProcess" -le "$stopDate" ]; do
	...
	draw_progress_bar $daysProcessed $daysToProcess "days: TII_DUMP_2HZ ${satellite} $datestring"
 	tii_dump_2hz ${satellite}${year}${month}${day} ${outDir} ${intervalSeconds}
