...
// Based on TII-Ion-Drift-Processor tiictParallel
...
#define ANOMALY_STATS_PARALLEL_SOFTWARE_VERSION "1.0"
...
#define ANOMALY_STATS_LABEL 0, 0
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            fprintf(stdout, "anomaly_stats_parallel version %s.\n", ANOMALY_STATS_PARALLEL_SOFTWARE_VERSION);
	...
	if (argc !=  6)
	{
		printf("usage:\t%s <startyyyymmdd> <endyyyymmdd> <efidir> <exportDir> <nthreads>\n\t\tparallel processes Swarm TII image anomaly statistics for specified satellite and date, looking for EFIxNOM and EFIxTIC files in efidir.\n", argv[0]);
	...
	sprintf(infoHeader, "anomaly_stats_parallel: ");
	...
	for (int sat = 0; sat < 3; sat++)
	{
		...
		queued =0;
		erase();
        mvprintw(ANOMALY_STATS_LABEL, "Processing anomaly statistics");
...
void *runThread(void *a)
{
	...
	// run anomaly_stats command as a system() call because CDF library is not thread safe
	int status = 0;
	char command[3*FILENAME_MAX+256] = {0};
    status = chdir(args->efiDir);
    if (status != 0)
    {
	...
	sprintf(command, "anomaly_stats %s%4d%02d%02d %s > /dev/null 2>&1 ", args->satLetter, args->year, args->month, args->day, args->exportDir);
